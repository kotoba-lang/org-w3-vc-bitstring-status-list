...
(defn ->doc
  "Encode `x`. Map keys must be keywords or strings; nil values are dropped."
  [x]
  (cond
    (string? x) ["string" x]
    (keyword? x) ["keyword" x]
    (boolean? x) ["bool" x]
    (integer? x) ["i64" x]
    (map? x) ["map" (mapv (fn [[k v]] [["keyword" (if (keyword? k) k (keyword k))]
                                       (->doc v)])
                          (sort-by (comp str key) (remove (comp nil? val) x)))]
